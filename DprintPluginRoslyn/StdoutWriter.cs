...
public sealed class StdoutWriter : IDisposable
{
    ...
    {
        SendMessage(GetNextSuccessResponse(originalMessageId));
    }

    public SuccessResponseMessage GetNextSuccessResponse(uint originalMessageId)
    {
        return new SuccessResponseMessage(_id.Next(), originalMessageId);
    }
